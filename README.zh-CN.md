...
Terra 不是兜底。若 Luna 的任一条件尚未建立，适用时先运行 compact plan 再选择执行器；缺少事实时返回 `NEEDS_CONTEXT` 并指出精确缺项；编辑前边界仍不清楚时走现有 Tier 3 歧义升级。补齐上下文或规划后重新评估边界，Tier 2 内只有命名例外可以选择 Terra。
